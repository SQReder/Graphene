...
const getInternalOwnershipEdges = (
	...
	const ownershipEdges = lookups.edgesBySource.ownership.get(fxNodeId) ?? [];
				...
				return {
					...
					data: {
						...
						relatedNodes: {
							...
							collapsed: [externalEdge.data.relatedNodes.source, ...(externalEdge.data.relatedNodes.collapsed ?? [])],
						},
					},
					style: {
...
export const foldEffects: GraphCleaner = (graph) => {
	...
	fxNodes.forEach((fxNode) => {
		try {
			const foldedEdges = foldEffect(graph, fxNode, lookups, nodeById);
